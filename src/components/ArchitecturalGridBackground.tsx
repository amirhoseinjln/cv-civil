...
export const ArchitecturalGridBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // 1. Ambient Illuminating Spotlight (Deep Terracotta & Blueprint Charcoal)
      ...
      radialGlow.addColorStop(0, 'rgba(222, 114, 71, 0.09)');
      radialGlow.addColorStop(0.35, 'rgba(222, 114, 71, 0.035)');
      radialGlow.addColorStop(0.75, 'rgba(20, 28, 44, 0.04)');
      ...
      if (mouse.pulseRadius > 0) {
        ...
        ctx.strokeStyle = `rgba(222, 114, 71, ${pulseAlpha})`;
        ctx.lineWidth = 1.5;
        ctx.stroke();
      }
      ...
      ctx.setLineDash([4, 8]);
      ctx.lineWidth = 0.8;
      ctx.strokeStyle = 'rgba(222, 114, 71, 0.14)';
      ...
      for (let col = 0; col < numCols; col++) {
        ...
        for (let row = 0; row < numRows; row++) {
          ...
          if (dist < 300) {
            ...
            ctx.lineWidth = 1;
            ctx.strokeStyle = `rgba(222, 114, 71, ${0.12 + factor * 0.45})`;
            ...
            if (factor > 0.5) {
              ctx.fillStyle = `rgba(234, 132, 90, ${factor * 0.6})`;
              ctx.beginPath();
            ...
            ctx.lineWidth = 0.75;
            ctx.strokeStyle = 'rgba(140, 160, 190, 0.09)';
            ctx.beginPath();
